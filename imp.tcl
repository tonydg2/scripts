# implementation
# required DCPs:  $outputDir/synth.dcp
#                 $rmDir/post_synth_$x.dcp (all RMs)
#
# Args passed in for this script: $RMs(list) $rmDir $outputDir $rpCell
...
foreach x $RMs {
  if {$idx==1} {
    ...
    read_checkpoint -cell $rpCell $rmDir/post_synth_$x.dcp
    opt_design
    place_design
    phys_opt_design
    route_design
...
#    write_checkpoint  -force $outputDir/post_route.dcp  ;#this is config1 (includes RM)
    write_checkpoint  -force $outputDir/config_$idx\_routed.dcp  ;#this is config1 (includes RM)
    #write_checkpoint  -force -cell $rpCell $outputDir/route_$x.dcp ;# this is only the current RM, don't need for now
    update_design     -cell $rpCell -black_box ;# this removes the current RM, so we can create static that has no RMs in it
    lock_design       -level routing
    write_checkpoint  -force $outputDir/static_route.dcp ;# true static, no RMs
    incr idx
  } else {
    open_checkpoint $outputDir/static_route.dcp
    read_checkpoint -cell $rpCell $rmDir/post_synth_$x.dcp ;# next RM
    opt_design
    place_design
    route_design
    write_checkpoint -force $outputDir/config_$idx\_routed.dcp ;# config2,etc.
    incr idx
  }
}

#  open_checkpoint $outputDir/post_route.dcp ;# should this be static_route?
